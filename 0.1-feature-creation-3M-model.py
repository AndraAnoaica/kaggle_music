...
def convert_date(x):
    ...
    return date
# list with users that generate errors
members_error = []
...
df_features_output_path = "/media/andraa/10160545101605452/kaggle/WSDM-kaggle/data/intermediate_data/df_features.csv"
...
for member_msno in members_list:
    if k <100:
        ...
        k+=1
    #    if k%10000 == 0:
        print("user number %d extracted in %d seconds" %(k, time.time()-start_time))
        ...
        if len(df_user) > 1 :
            ...
            # condition for the users that have days when they did not use the service
            if len(df_user.groupby(["msno", pd.TimeGrouper(freq = "M")]).count().index.levels[0]) == 2:
                ...
                #no merge to be done, groupby directly
                df_user_merge = df_user.groupby([pd.TimeGrouper(freq = "M")]).sum()
                df_user_merge["nb_days_no_music_month"] = 0   
                del df_user
            gc.collect()
            ...
            if len(df_user_merge) >=3 :
                ...
                if len(df_user_3_month.columns.values) == 8:
                    #print(df_user_3_month)
